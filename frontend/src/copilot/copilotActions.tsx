    ...
    useCopilotAction({
        ...
        handler: async ({ responseContent }) => {
            try {
                ...
                const { replyToCurrentEmail } = useUIStore.getState();
                // Ensure replyToCurrentEmail is available in your store
                if (replyToCurrentEmail) {
                    replyToCurrentEmail(current, responseContent);
                    return "Reply draft created.";
                }
                return "Reply function not available in store.";
    ...
    useCopilotAction({
        ...
        handler: async ({ to, message }) => {
            try {
                ...
                const { forwardCurrentEmail } = useUIStore.getState();
                if (forwardCurrentEmail) {
                    forwardCurrentEmail(current, message, to);
                    return "Forward draft created.";
                }
                return "Forward function not available in store.";
            } finally {
                setCopilotProcessing(false);
            }
        },
